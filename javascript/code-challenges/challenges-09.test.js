...
const hasChildrenValues = (arr, character) => {
  ...
  if (foundCharacter) {
    const children = Object.values(foundCharacter).includes(foundCharacter.children);
    return children && foundCharacter.children.length > 0;
  }
  return false;
};
...
const hasChildrenEntries = (arr, character) => {
  const foundCharacter = arr.find((char) => char.name === character);
  if (foundCharacter) {
    const entries = Object.entries(foundCharacter);
    const children = entries.find(([key, value]) => key === 'children' && Array.isArray(value) && value.length > 0);
