...
const longestString = (arr) => {
  if (arr.length === 0) {
    return -1; // Return a sentinel value indicating an empty array
  }
...
const sortByDay = (arr) => {
  const organizedEvents = Array.from({ length: 7 }, () => []);

  arr.forEach((event) => {
    const days = event.split(' and ');
    days.forEach((day) => {
      const matchingDays = daysOfWeek.filter((d) => day.includes(d));
      matchingDays.forEach((matchingDay) => {
        const index = daysOfWeek.indexOf(matchingDay);
        if (index !== -1) {
          organizedEvents[index].push(event);
        }
      });
    });
  });
