...
const count = (target, input) => {
  const count = input.reduce((accumulator, array) => {
    const occurrences = array.filter(num => num === target).length;
...
const totalSum = (input) => {
  const sum = input.reduce((accumulator, array) => {
    const arraySum = array.reduce((arrayAccumulator, num) => arrayAccumulator + num, 0);
...
let findMaleAndFemale = (data) => {
  ...
  const result = maleAndFemaleCharacters.join(' and ');
