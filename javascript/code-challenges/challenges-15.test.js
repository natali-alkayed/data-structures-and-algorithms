...
let biggerThanLuke = (arr) => {
  ...
  if (!lukeCharacter) {
    return ''; // Return an empty string if Luke Skywalker is not found
  }
...
const isSecure = (url) => {
  if (typeof url !== 'string') {
    return false; // Return false for non-string input
  }

  if (!url.startsWith('https://')) {
    return false; // Return false if the URL doesn't start with "https://"
  }

  if (url.indexOf('https://') !== url.lastIndexOf('https://')) {
    return false; // Return false if the URL has multiple occurrences of "https://"
  }
